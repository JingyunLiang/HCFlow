...
class FlowNet(nn.Module):
                ...
                if level == 0:
                    ...
                    self.level0_condFlow = ConditionalFlow(num_channels=self.C,
                                                    ...
                                                    opt=opt['network_G']['flowDownsampler']['splitOff'],
                                                               num_levels_condition=2, SR=True)
                elif level == 1:
                    self.layers.append(Basic.Split(num_channels_split=self.C // 2 if level < self.L-1 else 3, level=level))
                    ...
                    self.layers.append(Basic.Split(num_channels_split=self.C // 2 if level < self.L-1 else 3, level=level))
                    self.level2_condFlow = ConditionalFlow(num_channels=self.C,
                                                        num_channels_split=self.C // 2 if level < self.L-1 else 3,
                                                        n_flow_step=after_splitOff_flowStep[level],
                                                        opt=opt['network_G']['flowDownsampler']['splitOff'],
    ...
    # nodetach version: # nodetach version; 0.05 better than detach version, 0.30 better when using only nll loss
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            if isinstance(layer, FlowStep):
                ...
                if layer.level == 0:
                    ...
                    z, a3 = layer(z, reverse=False)

                    logdet, conditional_feature3 = self.level2_condFlow(a3, z, logdet=logdet, reverse=False, training=training)

                    conditional_feature2 = torch.cat([y2, F.interpolate(conditional_feature3, scale_factor=2, mode='nearest')],1)
                    logdet, conditional_feature2 = self.level1_condFlow(a2, conditional_feature2, logdet=logdet, reverse=False, training=training)

                    conditional_feature1 = torch.cat([y1, F.interpolate(conditional_feature2, scale_factor=2, mode='nearest'),
                                                      F.interpolate(conditional_feature3, scale_factor=4, mode='nearest')],1)
                    logdet, _ = self.level0_condFlow(a1, conditional_feature1, logdet=logdet, reverse=False, training=training)

        return z, logdet

    '''
    y3+z3(a3)->y2+z2(a2)->y1+z1(a1)->hr
    '''
    def reverse_flow(self, z, u=None, eps_std=None, training=True):
        for layer, shape in zip(reversed(self.layers), reversed(self.output_shapes)):
            if isinstance(layer, FlowStep):
                ...
                if layer.level == 2:
                    a3, _, conditional_feature3 = self.level2_condFlow(None, z, eps_std=eps_std, reverse=True, training=training)
                    z = layer(z, a3, reverse=True)
                elif layer.level == 1:
                    conditional_feature2 = torch.cat([z, F.interpolate(conditional_feature3, scale_factor=2, mode='nearest')],1)
                    a2, _, conditional_feature2 = self.level1_condFlow(None, conditional_feature2, eps_std=eps_std, reverse=True, training=training)
                    z = layer(z, a2, reverse=True)
                elif layer.level == 0:
                    conditional_feature1 = torch.cat([z, F.interpolate(conditional_feature2, scale_factor=2, mode='nearest'),
                                                      F.interpolate(conditional_feature3, scale_factor=4, mode='nearest')],1)
